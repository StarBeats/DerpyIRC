/*
	...
	Modified by LunaSquee to add extra support for DerpyIRC's requirements.
*/
...
var colors = require('./IRCColors');
exports.colors = colors;

var replyFor = require('./IRCResCodes');
...
function Client(server, nick, opt) {
    ...
    // Name of the network, if not defined, equals server address
    self.network = server;
...
Client.prototype.disconnect = function ( message, callback ) { 
    ...
    message = message || "DerpyIRC says goodbye";
    var self = this;
...
Client.prototype.sendraw = function(command) {
    if ( this.opt.debug )
        util.log('SEND-RAW: ' + args.join(" "));
